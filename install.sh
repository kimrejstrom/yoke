...
**Priority:** ■ Critical, ◆ High, ◇ Medium, ◦ Low'
    ...
    # --- Template: scripts/validate_docs.py ---
    write_template "scripts/validate_docs.py" '#!/usr/bin/env python3
# generated by yoke
"""Validate documentation cross-references.

Scans AGENTS.md, skill files, and docs for broken links to files that do not exist.
Outputs structured, agent-readable messages.
"""
...
def main() -> int:
        ...
        for line_num, target, link_type in links:
            ...
            if not resolved.exists():
                violations.append(
                    ...
                    f"Remediation: Update the link or create the missing file."
                )
    ...
    print("All documentation cross-references are valid.")
    return 0
  ...
  lint:
    ...
    steps:
      ...
      - name: Architecture lint
        run: python scripts/lint_architecture.py || true
      - name: Doc validation
        run: python scripts/validate_docs.py || true
  ...
  - repo: local
    hooks:
      ...
      - id: architecture-lint
        name: architecture lint
        entry: python scripts/lint_architecture.py
        language: system
        pass_filenames: false
      - id: test
        name: test
...
- `scripts/lint_architecture.py` - Architecture boundary linter
- `scripts/validate_docs.py` - Doc cross-reference validation
- `scripts/audit_principles.py` - Golden principles audit
